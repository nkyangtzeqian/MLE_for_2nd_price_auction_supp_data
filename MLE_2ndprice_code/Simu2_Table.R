...
for (i in 1:length(SimuList$Method)) {
  ...
  load(paste0("SIMU/HulC_TrunCov_", SimuList$Method[i], " With No. Auction = ", SimuList$Ka[i], ".RData"))
  Table2$Cov_MLE_HulC_OB[i] <- function_coverage(Mat_Cov_MLE)
  Table2$Cov_INIT_HulC_OB[i] <- function_coverage(Mat_Cov_Init)
  Table2$Cov_PT_Cred_OB[i] <- function_coverage(Mat_Cov_PT)
}
Table2