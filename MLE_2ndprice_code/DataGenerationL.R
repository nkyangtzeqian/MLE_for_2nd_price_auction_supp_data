...
Data_Gen_Raw <- function(Ka, tau_a, lambda0_list, r_k,
                                   method = c("unif", "pareto", "gamma", "beta",
                                              "piecewise_unif", "customized"),
                                   para1 = NULL, para2 = NULL, customized_CDF = NULL){
  # lambda0 <- lambda0_list[[1]]
  Lambda0 <- lambda0_list[[2]]
  Lambda0_inv <- lambda0_list[[3]]
  ...
  for (jj in 1: Ka) { ## for each auctions
    ...
    N_all[jj] <- rpois(1, Lambda0(tau_a)) # Total number of potential bids for the jth auction, might be smaller than r. Not observed.
    tts <- runif(n = N_all[jj], min = 0, max = tau_a) # All the inter-arrival times of the bidders for the jth process before transformation
    t_otts <- Lambda0_inv(sort(tts)) # All the inter-arrival times
    if(method == "unif"){
...
Data_Gen_Observed <- function(Raw_data_all_list){
  ...
  Z_iT_iA_i <- matrix(0, ncol=3, nrow=0) # Pooled data from ell + K standing prices from all the auctions (including the reserve prices)
  colnames(Z_iT_iA_i) <- c("pooled_rec_value", "pooled_wait_time", "pooled_cum_wait_time")
  Xbar_i <- NULL
  ...
  for (jj in 1: Ka) {
    ...
    if(sum(r_k[jj]<= all_bid_price_j) == 0){ ## M=O=1
      ...
      M_k[jj] <- 0
      Z_iT_iA_i <- rbind(Z_iT_iA_i, cbind(r_k[jj], tau_a, tau_a))
      ...
      Z_iT_iA_i <- rbind(Z_iT_iA_i, 
                      cbind(c(r_k[jj], xx[inds]),t_i,c(sum_ti, tau_a)) )
  ...
  Z_iT_iA_i <- Z_iT_iA_i[order(Z_iT_iA_i[,1]),,drop=F]
  K_sold <- K_sold[which(K_sold!=0)]
  
  ## help to do the calculation
  # S_bar <- match(X_mk[K_sold], Z_iT_i[ ,1])  # old version, only sold above
  S_bar <- match(X_mk[!is.na(X_mk)], Z_iT_iA_i[ ,1]) # Vector/Set of ranks/positions of the selling prices of all the auctions (where item is sold) in the pooled data set.
  u_i <- sort(match(Xbar_i, Z_iT_iA_i[ ,1]))  # This is vector: (u1, u2,...,uL).
