...
General_Log_Likelihood_PA <- function( data, lambdas, theta, lambda_set){
  
  ## unlist the lambda_list to get the two lambda functions
  lambda=lambda_list_gen(lambdas, lambda_set)[[1]]
  Lambda=lambda_list_gen(lambdas, lambda_set)[[2]]
  
  A_m_T <- data$Z_iT_iA_i[ ,3]-data$Z_iT_iA_i[ ,2] 
  aul_m_tul <- A_m_T[A_m_T>0] ## remove all reserve price, when a_i=t_i; otherwise it should not
  ## length of it should be equal to ell
  ...
  term_without_theta <- (ell*log(2)) + sum(log(Lambda(T_0_sold))) 
  if (ell != 0) term_without_theta <- term_without_theta + sum(log(lambda(aul_m_tul)))
  # print(paste("term without theta = ", term_without_theta))
  term_with_theta_1st_term <- -(sum((Lambda(data$Z_iT_iA_i[ ,3]) - Lambda(A_m_T)) * cumprod(theta))) + 
    sum( (Q_i_card * log(theta))[1:S_bar_max] )
