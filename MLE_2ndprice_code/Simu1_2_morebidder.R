...
source("Initialization_lambda_F_Simp.R")
Initialization_lambda_F <- Initialization_lambda_F_Simp
# two step optimization for MLE to avoid low convergence
MLE_2ndprice <- MLE_2ndprice_twosteps

#------------------------------------------------------------------------------------
## User Input: True lambda, Auction window, Number of independent auctions 'N.auction'
#------------------------------------------------------------------------------------

# replication <- 30 # replication numbers of each such event i.e. selling an
...
for(ii in 1:replication){
  ...
  if(any(is.na(Bayes_PT$F.y))) {
    warning("Polya Tree method failed for this replication, skipping...")
    distanceList$TV.pt[ii] <- NaN
  }else{
...
save(list="distanceList", file = paste0("SIMU/Table1_", method.in, "_Lbd=", lambda0, ".RData"))


#########################################################################################################
## why use MLE_2ndprice_twosteps? to avoid extremely slow convergence when lambda is large
## see instability boundary correction and ile  "Plots/Example_MLE_2step_for_large_lambda.RData"
